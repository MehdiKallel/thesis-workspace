...
class TumhashContract extends Contract {
	...
	//read private data from PDC
	async readPayloadPrivateDataCollectionHash(ctx, payloadId) {
		const privateData = await ctx.stub.getPrivateDataHash('privateCollection', payloadId);
		return privateData;
	}
	//read public data
	async readPayloadPublic(ctx, payloadId) {
		const data = await ctx.stub.getState(payloadId);
		return data;
	}
	//delete private data
