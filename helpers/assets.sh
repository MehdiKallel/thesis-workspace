...
do
	...
	do
		...
		do
			...
			echo "finished populating ledger with drugs"
			echo "************deleting caliper from  populator************"
			cd /home/ubuntu/HyperLedgerLab-2.0/scripts
			source caliper_delete.sh

			echo "****************populating ledger with orders****************"
                        cd /home/ubuntu/HyperLedgerLab-2.0/caliper/benchmarks/"$chaincodeId"
                        rm config.yaml
                        cp populatorOrderConfig/config.yaml ./
                        goal=$(($currentAsset/30))
                        cd /home/ubuntu/HyperLedgerLab-2.0/caliper/benchmarks/"$chaincodeId"/helpers
                        python3 scriptConfigAssetsUpdated.py $goal
                        cd ..
                        rm config.yaml
			...
			echo "************deleting caliper from order populator************"
                        cd /home/ubuntu/HyperLedgerLab-2.0/scripts
                        source caliper_delete.sh
			sleep 10s	
		
		
			echo "*****************setting up main function files*********************"			
			cd /home/ubuntu/HyperLedgerLab-2.0/caliper/benchmarks/"$chaincodeId"
                        rm config.yaml
			...
			echo "*******launching watcher and running main function benchmark*******"
			bash watcher.sh &	
			cd /home/ubuntu/HyperLedgerLab-2.0/scripts
			source caliper_run.sh "$chaincodeId"
